...
class Cli extends Common
{
    ...
    public function check_online(){
        ...
        $num = ceil($count/50);
        for ($i = 0; $i < $num; $i++) {
            $live = DB::name('Live')->field("live_id, user_id, play_img,title,room_id,lebel,intime,stream_key,room_id")->where(['live_status' => '1'])->limit($i*50,50)->select();
            if (empty($live)) break;
            foreach ($live as $k => $v) {
...
//                                'authName'=>""
        ...
        if($order) {
            ...
            foreach ($order as $k => $v) {
                ...
                foreach ($orderGoods as $key => $val) {
                    $sale_ratio = DB::name('goods')->where(["goods_id" => $val['goods_id']])->value('sale_ratio');
                        ...
                        if ($val['seller']) { //有销售者
                            ...
                            if ($anchor) {
                                if ($anchor['tv_id']) {//电视台主播
                                    $goods_settlement[$i]['seller_ratio'] = $sale_ratio;
    ...
    public function remind_info(){
        $count = Db::name('apply')->where(['is_shenhe'=>'2'])->count();
        file_put_contents('2.txt','faker444');
        $num = ceil($count/50);
        for ($i = 0; $i<$num; $i++){
            $res = DB::name('apply')->field("zhu_phone,apply_id,start_time,title,merchants_id")->where(['is_shenhe' => '2'])->limit($i*50,50)->select();
            if (empty($res)) break;
            foreach ($res as $k=>$v){
                $a = time()-$v['start_time'];
                $company_name = Db::name('merchants')->where(['gl_merchants_id'=>$v['merchants_id']])->value('company_name');
                if($a < 180){
                    $this->sendSMS($v['zhu_phone'],$v['title'],$company_name);
                }
            }
        }
     ...
     * 发送验证码
     */
    public function sendSMS($mobile,$title,$company_name){
        if (empty($mobile) || !preg_match('#^13[\d]{9}$|14^[0-9]\d{8}|^15[0-9]\d{8}$|^18[0-9]\d{8}|^17[0-9]\d{8}$#', $mobile)) {
            error("手机格式不正确");
        }else{
            $content = "您的商家名为".'「'.$company_name.'」'."下的活动名".'「'.$title.'」'."还有三分钟开播了。【体验购】";
        }
