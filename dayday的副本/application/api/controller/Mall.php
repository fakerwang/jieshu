...
//            }
    ...
    //商家的一些信息
    public function merchants_detail(){
        if (Request::instance()->isPost()) {
            $uid = input('uid');
            $merchants_id = input('merchants_id');//商家商户id
            if (!$merchants_id) error("商户店铺id不能为空");
            $where['gl_merchants_id'] = $merchants_id;
            $merchants = Db::name('merchants')->field('gl_merchants_id,company_name,business_img')->where($where)->find();
            if ($merchants) {
                if ($uid) {
                    ...
                    if ($check) {
                        ...
                        $merchants['is_follow'] = '2';
                    }
                } else {
                    $merchants['is_follow'] = '2'; //未关注
                }
            }
            $merchants['follow_num'] = Db::name('follow_merchants')
                ->where(['user_id2'=>$merchants_id,'is_delete'=>'1'])
                ->count();
            $merchants['live_num'] = Db::name('apply')
                ->where(['is_shenhe'=>'6','is_del'=>'0','merchants_id'=>$merchants_id])
                ->count();
    ...
    public function merchants_info()
    {
        if (Request::instance()->isPost()) {
            ...
            $merchants['merchants_address'] = $merchants['merchants_province'].''.$merchants['merchants_city'].''.$merchants['merchants_country'];
            $legal = [];
            $legal[] = $merchants['legal_img'];
            $legal[] = $merchants['legal_img2'];
            $legal[] = $merchants['legal_img3'];
            $legal[] = $merchants['legal_img4'];
            $legal[] = $merchants['legal_img5'];
            $legal = implode(',',$legal);
            $merchants['legal_img'] = $legal;
           return success($merchants);
        }
    }
	...
	public function merchants_dynamic()
	{
        if ($params = Request::instance()->isPost()) {
            ...
            $list = Db::name('live_store')->alias("a")
                    ->field("a.live_store_id,a.intime,b.end_time,b.title,b.apply_id,b.cover_img,b.class_id,b.status,c.business_img,b.is_shenhe,b.ren_num,a.play_number,c.company_name,a.live_id")
                    ->join("apply b",'a.room_id=b.room_id')
                    ->join("merchants c","b.merchants_id=c.gl_merchants_id")
                    ->where(['b.merchants_id'=>$merchants_id])
                    ->group('a.live_store_id')
                    ->limit(($p-1)*$pageSize,$pageSize)
                    ->select();
            foreach ($list as $k=>$v){
                $list[$k]['watch_nums'] = $v['ren_num'] + $v['play_number'];
                $list[$k]['tag']=Db::name('home_class')->where(['id'=>$v['status']])->value('title');
                $list[$k]['share_nums'] = Db::name('share_merchants')->where(['apply_id'=>$v['apply_id'],'is_del'=>'1'])->count();
                $list[$k]['end_time'] = date("Y-m-d",$v['intime']);
