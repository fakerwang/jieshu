...
//        }
     ...
     */
     public function user_list(){
		$user_id = input("uid");
		$data['follow_num'] = Db::name("follow_merchants")->where(["user_id"=>$user_id,'is_delete'=>'1'])->count();
		$data['share_num'] = Db::name("share_merchants")->where(["user_id"=>$user_id,'is_del'=>'1'])->count();
		success($data);
     }
    ...
    public function follow_merchants(){
        ...
        if(!$user_id2)      error("商家id参数错误");
        if ($uid == $user_id2) error("传值错误");
        $user_phone = input('user_phone');
        if(!$user_phone)      error("用户手机号参数错误");
        $user_name = input('user_name');
        if(empty($user_name))  $user_name = $user_phone;
        if($type =='1'){
            $live_id = input('live_id');
            if(!$live_id)      error("直播id参数错误");
            $check = Db::name('follow_merchants')->where(['user_id' => $uid, 'user_id2' => $user_id2])->find();
            if ($check) {
    ...
    public function user_follow(){
        ...
        $count = Db::name('follow_merchants')->alias('a')
            ->join('th_merchants b','a.user_id2 = b.gl_merchants_id')
            ->where($where)
            ->count();
        $list =  Db::name('follow_merchants')->alias('a')
            ->field('a.follow_id,b.gl_merchants_id,b.member_id,b.live_id,b.company_name,b.business_img,b.company_desc')
            ->join('th_merchants b','a.user_id2 = b.gl_merchants_id')
            ->where($where)
            ->order("a.intime desc")
            ->group('a.user_id2')
        ...
        foreach ($list as $k=>$v){
            $count = Db::name('share_merchants')->where(['merchants_id'=>$v['gl_merchants_id']])->count();
            $list[$k]['share_num'] = $count;
        }
        $page = ceil($count/$pagesize);
    ...
    public function add_share(){
        ...
        $re = Db::name('share_merchants')->where(['apply_id'=>$apply_id,'user_id'=>$uid])->find();
        if(!$re){
            $res = Db::name('share_merchants')->insert($data);
            if($res){
                success('添加成功');
            }else{
                error('添加失败');
	...
	public function user_share(){
		...
		$count = Db::name('share_merchants')->where($where)->count();
		$data = Db::name('share_merchants')->where($where)->limit(($p-1)*$pagesize,$pagesize)->select();
        $arr = [];
        foreach($data as $k=>$v) {
            $list = Db::name("apply")->alias('b')
                ->field('b.cover_img,b.is_shenhe,b.apply_id,b.merchants_id,b.class_id,b.status,b.title,b.start_time,b.end_time')
                ->order('b.end_time')
                ->where(['b.apply_id'=>$v['apply_id']])
                ->find();
            $re = Db::name('merchants')->where(['gl_merchants_id'=>$list['merchants_id']])->find();
            $list['company_name'] = $re['company_name'];
            $list['business_img'] = $re['business_img'];
            $list['company_desc'] = $re['company_desc'];
            $list['show_time'] =date("Y-m-d H:i:s",$v['create_time']);
            $list['follow_num'] =Db::name('follow_merchants')->where(['user_id2'=>$list['merchants_id']])->count();
            $list['share_num'] =Db::name('share_merchants')->where(['merchants_id'=>$list['merchants_id']])->count();
            $list['tag'] =Db::name('home_class')->where(['id'=>$list['status']])->value('title');
            $list['share_id'] = $v['share_id'];
            $list['watch_nums'] = '0';
            $list['share_url'] = config('domain') . '/api/Index/share_live?apply_id=' . $list['apply_id'] . "&room_id=''";
            if($list['is_shenhe']=='2'){
                $list['type'] ='2';
            }elseif ($list['is_shenhe']=='4'){
                $list['type'] ='1';
            }elseif ($list['is_shenhe']=='6'){
                $list['type'] ='3';
            }
            $timediff = $list['end_time']-$list['start_time'];
            //计算小时数
     ...
     * 删除用户分享记录
     */
    public function del_share_live(){
        $uid = input('uid');
        $share_id = input('share_id');
        $res = Db::name('share_merchants')->where(['user_id'=>$uid,'share_id'=>$share_id])->delete();
        if($res){
            success('删除成功');
        }else{
            error('删除失败');
	...
	public function watch_live(){
		...
		$count = Db::name('watch_live')->where($where)->count();
		$data = Db::name('watch_live')->where($where)->limit(($p-1)*$pagesize,$pagesize)->group('apply_id')->select();
		$arr = [];
		foreach($data as $k=>$v){
			$list = Db::name("apply")->alias('b')
					->field('b.cover_img,b.is_shenhe,b.apply_id,b.merchants_id,b.class_id,b.status,b.title,b.start_time,b.end_time')
     ...
     * 删除用户观看记录
     */
    public function del_watch_live(){
        $uid = input('uid');
        $watch_id = input('watch_id');
        $res = Db::name('watch_live')->where(['user_id'=>$uid,'watch_id'=>$watch_id])->delete();
        if($res){
            success('删除成功');
        }else{
            error('删除失败');
    ...
    public function report(){
        ...
        $data['report_content'] = input('report_content');
        if(!$data['report_content']) error('请填写举报内容');
        $data['create_time'] = time();
        $re = Db::name('report')->where(['user_id'=>$data['user_id'],'live_id'=>$data['live_id']])->find();
        if(!$re){
     ...
     *
     */
    public function  count_share_follow(){
        $uid = input('uid');
        $res['follow_num'] = Db::name('follow_merchants')->where(['user_id'=>$uid,'is_delete'=>'1'])->count();
        $res['share_num'] = Db::name('share_merchants')->where(['user_id'=>$uid])->count();
        success($res);
    }
    ...
    public function has_message(){
        ...
        $map['is_delete'] = '0';
        $map['state'] = '2';
        $check = Db::name('Message')->where(['type'=>'1','member_id'=>$uid])->column('system_notice_id');
        ...
        if($list){
            ...
            $count = Db::name('message')->where(['member_id'=>$uid,'is_read'=>'1'])->count();
            if($count){
                success('1');
            }else{
                success('2');
...
//        $p  ?   $p  :   $p = 1;
            ...
            case 1://系统消息
                ...
                if($check){
                    ...
                    if(empty($a)){
                        ...
                        foreach ($a as $k=>$v){
                            $notice = Db::name('system_notice')->field('id,is_read,intime,content as message,title')->where(['is_delete'=>'0','id'=>$v,'state'=>'2'])->find();
                            $arr[] = $notice;
                        }
                        $count = count($arr);
                        $page = ceil($count/$pagesize);
                    ...
                    if($check['is_del']=='1'){
                        $res = Db::name('system_notice')->field('id,is_read,intime,content as message,title')->where(['is_delete'=>'0','state'=>'2'])->select();
                        foreach ($res as $k=>$v){
                            if(strtotime($v['intime']) > $check['update_time']){
                                  $id = $v['id'];
                                  $a = Db::name('system_notice')->field('id,is_read,intime,content as message,title')->where(['is_delete'=>'0','id'=>$id,'state'=>'2'])->find();
                                  $arr[] = $a;
                                  $count = count($arr);
                                  $page = ceil($count/$pagesize);
                            }else{
    ...
    public function read_state(){
        ...
        if(!$re){
            if(!$res){
                $re = Db::name('system_notice')->where(['is_delete'=>'0','state'=>'2'])->select();
                foreach ($re as $k=>$v){
                    $arr[] = $v['id'];
                }
    ...
    public function feedback(){
        if(Request::instance()->isPost()){
            ...
            $data['member_name'] = input('member_name');
            $data['member_phone'] = input('member_phone');
