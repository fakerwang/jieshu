...
//                'name'=>$get_gradeinfo['name']
    ...
    public function show_viewer(){
        ...
        if(count($list)==0){
            ...
            success(["list" =>$list,"count"=>strval($count)]);
    }
    /**
     *直播列表(电商直播商户列表)
     */
        ...
        if(Request::instance()->isPost()){
            ...
            foreach ($list as $k=>$v){
                ...
                if(!$v["live_id"]){
                    ...
                    $fans_count = DB::name("Follow")->where(["user_id2"=>$uid])->count();
...
//            }
    ...
    public function give_gift(){
        ...
        if ($up_mebinfo) {
            ...
            $merchants = DB::name("Merchants")->where(["member_id"=>$user_id2,'platform_type'=>1])->find();
            //砖石赏票转化
            $change_scale = $system["convert_scale1"]/$system["convert_scale2"];
            //赏票兑换人民币的比例
            $cash_scale = $system["convert_scale4"]/$system["convert_scale3"];
            if($anchor || $merchants){
                if($anchor){
                    //电视台给主播的比例
                    ...
                    //电视台给商家的比例
                    $dashang_scale = $merchants["dashang_scale"];
                    $tv_id = $merchants["tv_id"];
                    $member_type = 2;
                }
                //获取平台给电视台（比例）
                $tv_scale = DB::name("television")->where(["tv_id"=>$tv_id])->value("dashang_scale");
                //电视台收益值
                $tv_money =$count_price*$change_scale*$tv_scale *(100-$dashang_scale)/10000;
                //主播收益值
                $anchor_money = $count_price*$change_scale*$tv_scale * $dashang_scale/10000;
                //更新主播收益
                $anchor_get_money = DB::name("member")->where(["member_id"=>$user_id2])->value('e_ticket') + $anchor_money;
                $amount =DB::name("member")->where(["member_id"=>$user_id2])->value("amount");
                $anchor_amount = sprintf("%.2f",($anchor_money*$cash_scale));
                //更新主播可兑换金额
                $anchor_get_amount = (float)$amount+(float)$anchor_amount;
                $up_anchor_money = DB::name("member")->where(['member_id' => $user_id2])->update(['e_ticket' => $anchor_get_money,'amount'=>$anchor_get_amount,'uptime' => time()]);
                //更新电视台收益
                $tv_get_money = DB::name("Television")->where(["tv_id"=>$tv_id])->value("e_ticket") + $tv_money;
                $up_tv_money  = DB::name("Television")->where(["tv_id"=>$tv_id])->update(['e_ticket'=>$tv_get_money,'update_time'=>date("Y-m-d H:i:s")]);
                if(!$up_anchor_money || !$up_tv_money){
                    Db::rollback();
                    error('点击太快!');
                }
                $scale = $tv_scale.','.$dashang_scale;
                $mer_data = [
                    "user_id" => $user["member_id"],
                    "live_id"=>$live_id,
                    "user_id2"=>$user_id2,
                    ...
                    'e_ticket' =>$anchor_money,
                    'tv_id'=>$tv_id,
                    'number'=>$num
                ];
                $give_gift_id = DB::name("give_gift")->insertGetId($mer_data);
                //获取电视台可以兑换的人民币
                $cash_money = sprintf("%.2f",($tv_money*$cash_scale));
                $tv_data = [
                    "tv_id"=>$tv_id,
                    "member_type"=>$member_type,
                    "e_ticket" =>$tv_money,
                    "cash_money" =>(float)$cash_scale,
                    "give_gift_id"=>$give_gift_id,
                    "create_time"=>date("Y-m-d",time())
                ];
                $tv_res = DB::name("television_earnings")->insert($tv_data);
                if ($tv_res){
                    DB::commit();
                    success('送礼成功!');
                }else{
                ...
                if($mers){
                    $anchor_scale = $mers["dashang_scale"]/100;
                }else{
                    //总后台设置的主播主播打赏打赏比
                    $anchor_scale = $system["change_scale"]/100;
                }
                //获取礼物转化为赏票数量
                $e_ticket = $count_price*$change_scale*$anchor_scale;
                //获取后台给主播分配的砖石数量
                $get_money = DB::name("member")->where(["member_id"=>$user_id2])->value('e_ticket') + $e_ticket;
                $up_live_info = DB::name("member")->where(['member_id' => $user_id2])->update(['e_ticket' => intval($get_money),'uptime' => time()]);
                if (!$up_live_info){
                    Db::rollback();
                    error('点击太快!');
                }
...
//***************************************************************提现相关部分*********************************************************************************//
    ...
    public function video_list(){
        ...
        $pagesize = input('pagesize');
        $pagesize   ?   $pagesize   :   $pagesize = 10;
        $count =  DB::name("video")->where(["is_del"=>1,"member_id"=>$member_id])->count();
        $list = DB::name("video")->field("title,video_img,url,watch_nums,video_id,date")->where(["is_del"=>1,"member_id"=>$member_id])
            ->limit(($p-1)*$pagesize,$pagesize)->select();
        if(empty($list)){
            $list=[];
        }
    ...
    public function tag_live_list(){
        ...
        $pageSize = empty($params["pagesize"]) ? 20 : $params["pagesize"];
        //获取主播分配的标签;
        $data["sheng"] = ['like',$city.'%'];
        $data["live_status"] = 1;
        $live_info = DB::name("live")
                    ->field("live_id,play_img,watch_nums,room_id,play_address,play_address_m3u8,date,address,user_id")
                    ->where($data)
                    ->order("intime desc")
                    ->limit(($page-1)*$pageSize,$pageSize)
                    ->select();
