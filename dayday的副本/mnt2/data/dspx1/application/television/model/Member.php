...
{
    //只读字段
    protected $readonly = ['member_id','alias','hx_username','hx_password','wx_openid','qq_openid','wo_openid','tw_openid','fa_openid'];
        ...
        switch ($params["member_type"]) {
            case "anchor":
                if(empty($params["member_id"])){
                    ...
                    if($ares || $mres){
                        return success(['info'=>$action.'主播操作失败','url'=>$url]);
                    }else{
                        return error($action.'主播操作失败');
                    }
                }
