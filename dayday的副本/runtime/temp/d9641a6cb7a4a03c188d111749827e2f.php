...
<div class="page-container">
    <div class="text-l">
        <form class="search"  method="get">
            <input type="text" class="input-text" style="width:250px" placeholder="输入手机号" value="<?php echo !empty($_GET['username'])?$_GET['username']:''; ?>" name="username">
    ...
    <div class="cl pd-5 bg-1 bk-gray mt-20">
		<span class="l">
			...
			<a href="<?php echo url('Role/add_control'); ?>"  class="btn btn-primary radius">
				<i class="Hui-iconfont">&#xe600;</i> 添加场控
			</a>
            <span style="margin-left: 60px">场控共有<?php echo $count; ?>名</span>
            <span style="margin-left: 60px">正在直播<?php echo $count1; ?>名</span>
            <!--<span style="margin-left: 60px">剩余名额<?php echo $count1; ?>名</span>-->
		</span>
    ...
    <div class="mt-20">
        <table class="table table-border table-bordered table-hover table-bg table-sort">
            ...
            <tr class="text-c">
                <td><input type="checkbox" value="<?php echo $vo['control_id']; ?>" name="checkbox"></td>
                <td><?php echo $vo['anchor_id']; ?></td>
                <td><?php echo $vo['anchor_phone']; ?></td>
                <td><?php echo $vo['anchor_name']; ?></td>
                <td>
                    <?php switch($name=$vo['status']): case "0": ?>在线<?php break; case "1": ?>离线<?php break; endswitch; ?>
                </td>
                <td><?php echo date("Y-m-d H:i:s",$vo['create_time']); ?></td>
                <td>
                    <a title="编辑" href="<?php echo url('role/edit_control',['anchor_id'=>$vo['anchor_id']]); ?>"  class="ml-5" style="text-decoration:none"><i class="Hui-iconfont">&#xe6df;</i></a>
...
<script type="text/javascript">
    ...
    function datadel(kid){
        ...
        layer.confirm('确认要删除吗？',function(index){
            $.post("<?php echo url('Role/del_control'); ?>", {ids:kid}, function(data){
                if( data.status == 'ok' ){
    ...
    function del(obj,id){
        layer.confirm('确认要删除吗？',function(index){
            $.post("<?php echo url('Role/del_control'); ?>", {ids:id}, function(data){
                if( data.status == 'ok' ){
